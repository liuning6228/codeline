...
import { MCPToolAdapter } from '../core/tool/MCPToolAdapter';
...
export class MCPHandler {
  ...
  constructor(private context: vscode.ExtensionContext, config: Partial<MCPHandlerConfig> = {}) {
    // 设置默认配置
    this.config = {
      enableMCPTools: true,
   ...
   * 验证MCP消息格式
   */
  private validateMCPMessage(message: MCPMessage): string | null {
    if (!message) {
   ...
   */
  private async registerMCPTools(toolRegistry: any): Promise<number> {
    if (!this.mcpManager) {
      return 0;
    }
    ...
    for (const mcpTool of mcpTools) {
      try {
        ...
        const { MCPToolWrapper } = await import('../core/tool/MCPToolWrapper');
        ...
        if (registered) {
          ...
          const adapter = new MCPToolAdapter<Record<string, any>, any>(mcpTool, {
            enablePermissionCheck: true,
    ...
    if (registeredCount === 0 && mcpTools.length > 0) {
      ...
      try {
        const { EnhancedToolRegistryMCPExtension } = await import('../core/tool/EnhancedToolRegistryMCPExtension');
   ...
   * 处理获取指标请求
   */
  private async handleGetMetrics(): Promise<MCPResponse> {
    return {
      success: true,
   ...
   * 处理获取统计信息请求
   */
  private async handleGetStatistics(): Promise<MCPResponse> {
    return {
      success: true,
   ...
   * 处理配置操作请求
   */
  private async handleConfigOperation(data: any): Promise<MCPResponse> {
    ...
    switch (operation) {
      case 'get':
        if (key) {
          return {
            success: true,
            data: {
              [key]: this.config[key as keyof MCPHandlerConfig]
            }
          };
        } else {
          return {
      ...
      case 'set':
        ...
        const validKeys: (keyof MCPHandlerConfig)[] = [
          'enableMCPTools',
          'enableToolSystem',
          'verboseLogging',
          'defaultTimeout',
        ...
        if (!validKeys.includes(key as keyof MCPHandlerConfig)) {
          return {
            success: false,
      ...
      case 'reset':
        // 重置为默认值
        const defaultConfig: MCPHandlerConfig = {
          enableMCPTools: true,
          enableToolSystem: true,
          verboseLogging: false,
          defaultTimeout: 30000,
   ...
   * 设置配置监听器
   */
  private async setupConfigWatcher(workspaceRoot: string): Promise<void> {
    if (!this.config.autoLoadConfigPath) {
      return;
    }
    ...
    try {
      ...
      this.configWatcher = vscode.workspace.createFileSystemWatcher(
        new vscode.RelativePattern(configDir, path.basename(configPath))
      );
  ...
  private async reloadConfig(configPath: string): Promise<void> {
    try {
      ...
      const changedKeys = Object.keys(configJson).filter(key => {
        const typedKey = key as keyof MCPHandlerConfig;
   ...
   * 更新请求跟踪
   */
  private trackRequest(messageId: string, type: string, success: boolean, duration: number): void {
   ...
   * 开始工具执行跟踪
   */
  private startToolExecution(executionId: string, toolId: string, params: any): void {
   ...
   * 结束工具执行跟踪
   */
  private endToolExecution(executionId: string, success: boolean): void {
