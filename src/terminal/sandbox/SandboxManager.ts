...
import { TerminalProcess, TerminalProcessConfig } from '../executor/TerminalProcess';
...
export class SandboxManager {
  ...
  public async executeInSandbox(
    command: string,
    options?: Partial<TerminalProcessConfig>
    ...
    try {
      ...
      // 配置进程
      const processConfig: TerminalProcessConfig = {
      ...
      // 启动进程
      const process = new TerminalProcess(processConfig);
   ...
   * 创建受限文件系统视图
   */
  public async createRestrictedFSView(targetDir: string): Promise<string> {
  ...
  private async cleanupTempWorkspace(dir: string): Promise<void> {
    try {
