...
export class CommandParser {
  ...
  private static readonly COMMAND_SEMANTICS: Record<string, Partial<CommandSemantic>> = {
    // 文件操作
   ...
   * 分析命令语义
   */
  public analyzeSemantics(parsed: ParsedCommand): CommandSemantic {
    ...
    const baseSemantic = CommandParser.COMMAND_SEMANTICS[command] || {
      ...
      systemImpact: false,
      suggestedSandboxLevel: 'medium' as const
    };
    ...
    // 确定沙箱级别
    let suggestedSandboxLevel: 'none' | 'low' | 'medium' | 'high' = 'medium';
    if (riskLevel >= 8) {
   ...
   * 验证命令安全性
   */
  public validateSafety(parsed: ParsedCommand): {
    safe: boolean;
    warnings: string[];
    riskLevel: number;
  } {
   ...
   * 获取命令建议
   */
  public getSuggestions(parsed: ParsedCommand): string[] {
  ...
  private extractOptions(parsed: ParsedCommand): void {
  ...
  private extractRedirects(parsed: ParsedCommand): void {
  ...
  private extractPipeline(parsed: ParsedCommand): void {
