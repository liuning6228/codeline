...
import { CodeContextEnhancer, QueryContext, FileAnalysis } from './CodeContextEnhancer';
...
export interface CompletionConfig {
  ...
  languageConfigs: Record<string, {
    keywords: string[];
    commonSnippets: Array<{ label: string; insertText: string }>;
...
export class SmartCodeCompleter {
  ...
  private debugAnalyzer?: DebugAnalyzerTool;
   ...
   * 分析上下文
   */
  private async analyzeContext(context: EditorContext): Promise<{
    language: string;
    ...
    if (this.config.contextDepth !== 'shallow' && fs.existsSync(filePath)) {
      try {
        queryContext = await this.contextEnhancer.getQueryContext(
          `Completions at line ${cursorPosition.line + 1}, character ${cursorPosition.character + 1}`
        );
      } catch (error) {
        // 忽略上下文分析错误
      }
   ...
   * 检测编程语言
   */
  private detectLanguage(filePath: string, editorLanguageId?: string): string {
    // 优先使用编辑器语言ID
    if (editorLanguageId) {
   ...
   * 获取当前单词
   */
  private getCurrentWord(line: string, cursorPosition: number): string {
    // 找到光标前的单词开始位置
   ...
   * 检查是否为导入语句
   */
  private isImportStatement(linePrefix: string, language: string): boolean {
   ...
   * 检查是否为类型注解
   */
  private isTypeAnnotation(linePrefix: string, language: string): boolean {
   ...
   * 检查是否为函数调用
   */
  private isFunctionCall(linePrefix: string, language: string): boolean {
    const prefix = linePrefix.trim();
    return prefix.endsWith('(') || prefix.endsWith('.');
  }
   ...
   * 检查是否为属性访问
   */
  private isPropertyAccess(linePrefix: string, language: string): boolean {
    const prefix = linePrefix.trim();
   ...
   * 生成基础补全项
   */
  private async generateBasicCompletions(
    context: EditorContext,
    analysis: any
   ...
   * 生成导入补全项
   */
  private async generateImportCompletions(
    context: EditorContext,
    analysis: any
  ): Promise<CompletionItem[]> {
    const items: CompletionItem[] = [];
    ...
    // 常见的导入项
    const commonImports: Record<string, Array<{ name: string; description: string }>> = {
      typescript: [
   ...
   * 生成函数建议
   */
  private async generateFunctionSuggestions(
    context: EditorContext,
    queryContext: QueryContext
  ): Promise<CompletionItem[]> {
    const items: CompletionItem[] = [];
   ...
   * 生成类型建议
   */
  private async generateTypeSuggestions(
    context: EditorContext,
    analysis: any
  ): Promise<CompletionItem[]> {
    const items: CompletionItem[] = [];
  ...
  private async generateErrorFixes(
    ...
    try {
      ...
      const commonFixes: CompletionItem[] = [
        {
  ...
  private filterAndSortCompletions(
    ...
    filtered.sort((a, b) => {
      ...
      // 最后按类型排序
      const typeOrder: Record<CompletionType, number> = {
        'keyword': 0,
        'import': 1,
        'type': 2,
        'function': 3,
  ...
  private getAnalyzersUsed(): string[] {
    const analyzers: string[] = ['context'];
 ...
 * 创建默认的智能代码补全器
 */
export function createSmartCodeCompleter(
  workspaceRoot: string,
  options?: {
