...
export class EditorCommands {
  constructor(private codeLine: CodeLineExtension) {}

  /**
   * Add selected text to chat (Cline's addToChat command)
   */
  public async addToChat(): Promise<void> {
   ...
   */
  public async improveCode(): Promise<void> {
    const editor = vscode.window.activeTextEditor;
    if (!editor) {
      vscode.window.showInformationMessage('No active editor');
      return;
   ...
   */
  public async generateGitCommitMessage(): Promise<void> {
    ...
    try {
      ...
      const repository = git.repositories.find((repo: any) => repo.rootUri.fsPath === workspacePath);
      ...
      if (stagedChanges && stagedChanges.length > 0) {
        ...
        stagedChanges.forEach((change: any) => {
      ...
      if (unstagedChanges && unstagedChanges.length > 0) {
        ...
        unstagedChanges.forEach((change: any) => {
          changes.push(`- ${change.uri.fsPath.split('/').pop()} (${this.getGitChangeType(change.status)})`);
        });
      }
   ...
   */
  private getGitChangeType(status: number): string {
