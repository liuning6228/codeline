...
import { Tool, ToolContext, ToolProgress } from '../../tools/ToolInterface';
import { PluginMetadata, PluginContext, PluginConfig, PluginConfigSchema, MCPPlugin } from '../PluginInterface';

/**
 * 示例MCP插件配置
 */
interface ExampleMCPPluginConfig {
  /** 启用模拟MCP服务器 */
...
export class ExampleMCPPlugin implements MCPPlugin {
   ...
   * 获取MCP服务器列表
   */
  public async getMCPServers(): Promise<Array<{
    id: string;
    name: string;
    description: string;
    version?: string;
   ...
   * 连接MCP服务器
   */
  public async connectMCPServer(serverId: string, configuration?: Record<string, any>): Promise<void> {
    if (!this.context) {
   ...
   * 初始化模拟服务器
   */
  private async initializeMockServers(): Promise<void> {
    if (!this.context) {
      return;
    }
   ...
   * 仅供内部使用
   */
  private async executeMockTool(serverId: string, toolId: string, params?: Record<string, any>): Promise<any> {
    const server = this.servers.get(serverId);
    if (!server) {
      throw new Error(`Server ${serverId} not found`);
    }
    ...
    const tool = server.tools?.find((t: any) => t.id === toolId);
    if (!tool) {
