...
import { Tool, ToolContext, ToolDefinition, ToolResult, ToolProgress } from '../../tools/ToolInterface';
import { ToolPlugin, PluginMetadata, PluginContext, PluginConfig, PluginConfigSchema } from '../PluginInterface';

/**
 * 示例工具插件配置
 */
interface ExampleToolPluginConfig {
  /** 是否启用问候工具 */
  enableGreetingTool: boolean;
  /** 是否启用计算工具 */
  enableCalculatorTool: boolean;
  /** 默认问候语 */
 ...
 * 示例工具插件
 */
export class ExampleToolPlugin implements ToolPlugin {
  ...
  private config: ExampleToolPluginConfig;
   ...
   * 创建工具
   */
  private async createTools(): Promise<void> {
    this.tools.clear();
   ...
   * 重新创建工具
   */
  private async recreateTools(): Promise<void> {
    if (this.context) {
   ...
   * 创建问候工具
   */
  private createGreetingTool(): void {
    const tool: Tool = {
      id: 'example-greeting',
      name: '示例问候工具',
      description: '根据配置返回问候语',
      ...
      execute: async (params: Record<string, any>, context: ToolContext, onProgress?: (progress: ToolProgress) => void) => {
   ...
   * 创建计算工具
   */
  private createCalculatorTool(): void {
    const tool: Tool = {
      id: 'example-calculator',
      name: '示例计算工具',
      description: '执行简单的数学计算',
      ...
      parameterSchema: {
        operation: {
          ...
          required: true,
          validation: (value: any) => ['add', 'subtract', 'multiply', 'divide'].includes(value),
        },
        a: {
          type: 'number',
          description: '第一个操作数',
