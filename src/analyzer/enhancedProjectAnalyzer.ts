...
export class EnhancedProjectAnalyzer extends ProjectAnalyzer {
   ...
   */
  public override async analyzeCurrentWorkspace(): Promise<ProjectContext> {
    // 调用父类方法保持兼容
   ...
   * 分析当前工作区，返回增强的上下文信息
   */
  public async analyzeEnhancedWorkspace(focusedFile?: string): Promise<EnhancedProjectContext> {
    ...
    try {
      // 并行执行基础分析和增强分析
      const [baseContext, dependencyGraph, moduleRelations, codeQuality, architecturePatterns, relevantContext] = 
        await Promise.all([
      ...
      // 构建增强的上下文
      const enhancedContext: EnhancedProjectContext = {
        ...baseContext,
        dependencyGraph,
        moduleRelations,
        codeQuality,
   ...
   * 分析依赖关系图
   */
  private async analyzeDependencyGraph(): Promise<DependencyGraph> {
    const nodes: any[] = [];
    const edges: any[] = [];
   ...
   * 分析模块关系（导入/导出）
   */
  private async analyzeModuleRelations(): Promise<ModuleRelation[]> {
   ...
   * 检测代码问题
   */
  private detectCodeIssues(file: string, content: string, issues: CodeIssue[]): void {
    const lines = content.split('\n');
   ...
   * 生成重构建议
   */
  private generateRefactoringSuggestions(file: string, content: string, suggestions: RefactoringSuggestion[]): void {
    // 检测长函数
    const lines = content.split('\n');
   ...
   * 估计重复率（简化的基于行的重复检测）
   */
  private async estimateDuplicationRate(codeFiles: string[]): Promise<number> {
  ...
  private calculateOverallScore(metrics: CodeMetrics, issues: CodeIssue[]): number {
    ...
    if (metrics.duplicationRate > 0.3) score -= 25;
    else if (metrics.duplicationRate > 0.2) score -= 15;
    else if (metrics.duplicationRate > 0.1) score -= 5;
   ...
   * 检测分层架构
   */
  private async detectLayeredArchitecture(): Promise<string[]> {
    const layers: string[] = [];
   ...
   * 检查微服务指标
   */
  private async checkMicroserviceIndicators(): Promise<boolean> {
   ...
   * 收集相关上下文
   */
  private async collectRelevantContext(focusedFile?: string): Promise<RelevantContext> {
    ...
    try {
      ...
      if (focusedFile) {
        ...
        for (const file of allFiles) {
          ...
          let relationType: RelatedFile['relationType'] = 'similar-code';
   ...
   * 收集代码文件
   */
  private async collectCodeFiles(rootPath: string): Promise<string[]> {
   ...
   * 检查是否存在指定目录或文件
   */
  private async hasDirectoryOrFiles(names: string[]): Promise<boolean> {
   ...
   * 获取目录中的文件列表
   */
  private async getFilesInDirectories(dirs: string[]): Promise<string[]> {
    const files: string[] = [];
   ...
   * 确定分析深度
   */
  private determineAnalysisDepth(context: ProjectContext): 'shallow' | 'medium' | 'deep' {
   ...
   * 获取默认的增强上下文
   */
  private getDefaultEnhancedContext(): EnhancedProjectContext {
