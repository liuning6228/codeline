...
import { MCPManager, MCPResult, MCPOptions, MCPTool } from '../../mcp/mcpManager';
 ...
 * MCP管理器适配器
 */
export class MCPManagerAdapter extends BaseToolAdapter<MCPToolParams, MCPToolResultData> {
  ...
  constructor(context: ToolContext) {
    super(
      'mcp-manager',
      'MCP Manager',
      'Manage and execute Model Context Protocol (MCP) tools',
      '1.0.0',
      'CodeLine Team',
      ['mcp', 'tools', 'protocol', 'model-context', 'integration'],
      {
        operation: {
          type: 'string',
   ...
   * 检查权限 - MCP工具需要特别注意
   */
  async checkPermissions(params: MCPToolParams, context: ToolContext): Promise<PermissionResult> {
   ...
   * 验证参数
   */
  async validateParameters(params: MCPToolParams, context: ToolContext): Promise<ValidationResult> {
    ...
    // 清理参数
    const sanitizedParams: MCPToolParams = {
      ...params,
  ...
  async execute(
  ...
  ): Promise<ToolResult<MCPToolResultData>> {
    ...
    try {
      let resultData: MCPToolResultData = {};
