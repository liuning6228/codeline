...
var WorktreesView = function (_a) {
    ...
    ]), snapshots = _b[0], setSnapshots = _b[1];
    var _c = (0, react_1.useState)('snap-1'), selectedSnapshot = _c[0], setSelectedSnapshot = _c[1];
    // 创建新快照
    var handleCreateSnapshot = function () {
        if (snapshots.length >= config.maxSnapshots) {
            alert("Maximum ".concat(config.maxSnapshots, " snapshots allowed"));
            return;
        }
        var newSnapshot = {
            id: "snap-".concat(snapshots.length + 1),
            name: "Snapshot ".concat(snapshots.length + 1),
        ...
        setSnapshots(function (prev) { return __spreadArray([newSnapshot], prev, true); });
        setSelectedSnapshot(newSnapshot.id);
    };
    // 删除快照
    var handleDeleteSnapshot = function (snapshotId) {
        setSnapshots(function (prev) { return prev.filter(function (snap) { return snap.id !== snapshotId; }); });
        if (selectedSnapshot === snapshotId) {
            setSelectedSnapshot(snapshots.length > 1 ? snapshots[1].id : null);
        }
    };
    // 格式化文件大小
    var formatFileSize = function (kb) {
        if (kb < 1024)
            return "".concat(kb, " KB");
    ...
    var selectedSnapshotData = snapshots.find(function (snap) { return snap.id === selectedSnapshot; });
    if (!config.enabled) {
    ...
    return (<div className="flex h-full flex-col p-6">
      ...
      <div className="mb-8">
        ...
        <div className="mt-4 flex items-center space-x-4">
          ...
          <div className="text-sm text-gray-400">
            {formatFileSize(snapshots.reduce(function (total, snap) { return total + snap.size; }, 0))} total
          </div>
          {config.autoSnapshotInterval > 0 && (<div className="text-sm text-gray-400">
      ...
      <div className="flex flex-1 space-x-6 overflow-hidden">
        ...
        <div className="w-1/3 flex flex-col space-y-4">
          <div className="flex justify-between items-center">
            ...
            <button onClick={handleCreateSnapshot} className="px-3 py-1.5 bg-primary text-primary-foreground rounded-md text-sm hover:bg-primary/90" disabled={snapshots.length >= config.maxSnapshots} title={snapshots.length >= config.maxSnapshots ? "Maximum ".concat(config.maxSnapshots, " snapshots allowed") : ''}>
              Create Snapshot
            </button>
          </div>
          
          <div className="flex-1 overflow-auto space-y-3">
            {snapshots.length === 0 ? (<div className="text-center py-8 text-gray-400">
                <div className="text-lg mb-2">No snapshots yet</div>
                <p className="text-sm">Create your first workspace snapshot</p>
              </div>) : (snapshots.map(function (snapshot) { return (<div key={snapshot.id} className={"p-4 rounded-lg border cursor-pointer transition-all ".concat(selectedSnapshot === snapshot.id
                ? 'border-primary bg-primary/10'
                : 'border-border hover:border-primary/50')} onClick={function () { return setSelectedSnapshot(snapshot.id); }}>
                  <div className="flex justify-between items-start mb-2">
                    ...
                    <div className="flex items-center">
                      <span className={"px-2 py-0.5 text-xs rounded ".concat(snapshot.type === 'manual' ? 'bg-blue-500/20 text-blue-600' :
                snapshot.type === 'auto' ? 'bg-green-500/20 text-green-600' :
                  ...
                  <div className="flex justify-between items-center">
                    <div className="flex space-x-2">
                      {snapshot.tags.map(function (tag) { return (<span key={tag} className="px-2 py-0.5 bg-secondary text-xs rounded">
                          {tag}
                        </span>); })}
                    </div>
        ...
        <div className="w-2/3 flex flex-col space-y-6">
          {selectedSnapshotData ? (<>
              ...
              {/* 差异比较（如果启用） */}
              {config.showDiffView && snapshots.length > 1 && (<div className="flex-1 flex flex-col">
                  ...
                  <div className="flex-1 bg-secondary/30 p-4 rounded-lg">
                    <div className="flex items-center space-x-4 mb-4">
                      <select className="px-3 py-2 bg-background border border-border rounded">
                        ...
                        {snapshots
                    .filter(function (snap) { return snap.id !== selectedSnapshotData.id; })
                    .map(function (snap) { return (<option key={snap.id} value={snap.id}>
