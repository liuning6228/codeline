...
exports.containsExcludedCommand = containsExcludedCommand;
/**
 * 判断是否应该在沙箱中执行命令
 */
 ...
 * 检查是否包含危险操作
 */
function containsDangerousOperation(command) {
 ...
 * 检查是否修改文件系统
 */
function modifiesFileSystem(command) {
 ...
 * 检查是否执行外部程序
 */
function executesExternalProgram(command) {
...
function isComplexCommand(command) {
    ...
    // 检查命令复杂度（特殊字符数量）
    const specialChars = ['|', '&', ';', '>', '<', '`', '$', '(', ')', '{', '}'];
    const specialCharCount = specialChars.reduce((count, char) => count + (command.split(char).length - 1), 0);
...
//# sourceMappingURL=shouldUseSandbox.js.map