...
exports.CodeContextEnhancer = void 0;
...
class CodeContextEnhancer {
     ...
     * 获取当前查询上下文
     */
    async getQueryContext(userInput) {
     ...
     * 提取相关代码片段
     */
    async extractRelevantCodeSnippets(query, maxSnippets = 5) {
        try {
            // 获取项目结构
     ...
     * 执行文件分析
     */
    async performFileAnalysis(document) {
        const filePath = document.uri.fsPath;
     ...
     * 查找相关文件
     */
    async findRelevantFiles(keywords, project) {
        const relevantFiles = [];
     ...
     * 从文件中提取片段
     */
    async extractSnippetsFromFile(file, keywords) {
        const snippets = [];
        try {
            const content = await fs.readFile(file.path, 'utf-8');
            const lines = content.split('\n');
     ...
     * 分析文件变更影响
     */
    async analyzeFileChangeImpact(change, project) {
        // 简化实现
        return {
    ...
    shouldIgnore(name) {
        ...
        return ignorePatterns.some(pattern => name === pattern || name.startsWith(pattern + '.') || name.endsWith('.' + pattern));
    }
    ...
    // 各种语言的解析方法（简化实现）
    async analyzeJavaScriptLikeFile(content, analysis) {
        // 简化实现：使用正则表达式提取基本信息
        const lines = content.split('\n');
        // 提取导入语句
    ...
    async analyzePythonFile(content, analysis) {
        // Python文件分析简化实现
    }
    async analyzeJavaFile(content, analysis) {
        // Java文件分析简化实现
    }
    async analyzeGoFile(content, analysis) {
        // Go文件分析简化实现
    }
    async analyzeRustFile(content, analysis) {
        // Rust文件分析简化实现
    }
    async analyzeGenericFile(content, analysis) {
        // 通用文件分析简化实现
    }
    /**
     * 分析指定文件（兼容性方法）
...
exports.CodeContextEnhancer = CodeContextEnhancer;
exports.default = CodeContextEnhancer;
//# sourceMappingURL=CodeContextEnhancer.js.map