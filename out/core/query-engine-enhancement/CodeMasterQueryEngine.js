...
exports.CodeMasterQueryEngine = void 0;
 ...
 * 为编码任务专门增强的查询引擎
 */
class CodeMasterQueryEngine extends EnhancedQueryEngine_1.EnhancedQueryEngine {
    codeEnhancer;
    codeConfig;
    codingTools = new Map();
    stateMemory = new StateMemory();
    ...
    constructor(config) {
        ...
        // 初始化代码上下文增强器
        this.codeEnhancer = new CodeContextEnhancer_1.CodeContextEnhancer(config.workspaceRoot);
        // 初始化编码专用工具
    ...
    async *submitMessage(content, options = {}) {
        ...
        try {
            ...
            const responseContent = await this.generateEnhancedResponse(content, userIntent, codeContext, toolCalls, enhancedThinking);
            // 6. 更新状态记忆
     ...
     * 增强型思考响应
     */
    async enhancedThinkAboutResponse(userMessage, intent, context) {
        // 构建增强型思考提示
...
6. **Implementation steps**:`;
     ...
     * 智能工具解析
     */
    async intelligentToolParsing(userMessage, intent, context, options) {
        const requirements = [];
        const parameters = {};
        const errors = [];
        const warnings = [];
     ...
     * 生成增强响应
     */
    async generateEnhancedResponse(userMessage, intent, context, toolCalls = [], enhancedThinking = '') {
        // 构建增强响应提示
...
Please provide a helpful response that addresses the user's request.`;
     ...
     * 更新状态记忆
     */
    async updateStateMemory(userInput, assistantResponse, intent, toolCalls) {
        const memoryEntry = {
            timestamp: Date.now(),
            userInput,
     ...
     * 从工具需求创建工具调用
     */
    createToolCallFromRequirement(requirement, parameters) {
        return {
            id: this.generateId(),
    ...
    extractConstraints(userInput) {
        ...
        const constraintPatterns = [
            { pattern: /without\s+(\w+)/gi, extract: (match) => `Without ${match}` },
            { pattern: /using\s+(\w+)/gi, extract: (match) => `Using ${match}` },
            { pattern: /must\s+(\w+)/gi, extract: (match) => `Must ${match}` },
            { pattern: /should\s+(\w+)/gi, extract: (match) => `Should ${match}` },
            { pattern: /cannot\s+(\w+)/gi, extract: (match) => `Cannot ${match}` },
            { pattern: /need\s+to\s+(\w+)/gi, extract: (match) => `Need to ${match}` },
            { pattern: /constraint:?\s*(\w+)/gi, extract: (match) => `Constraint: ${match}` },
        ];
     ...
     * 确定优先级
     */
    determinePriority(userInput, intentType) {
        const lowerInput = userInput.toLowerCase();
        // 高优先级关键词
     ...
     * 识别所需工具
     */
    identifyRequiredTools(intentType, operation, context) {
        const tools = [];
        switch (intentType) {
            case 'code_generation':
     ...
     * 生成建议方法
     */
    generateSuggestedApproach(intentType, operation, targetFiles) {
     ...
     * 解析思考步骤
     */
    parseThinkingSteps(thinkingContent) {
     ...
     * 解析代码生成工具
     */
    async parseCodeGenerationTools(intent, context) {
        const requirements = [];
        // 基础代码生成工具
        requirements.push({
     ...
     * 解析代码编辑工具
     */
    async parseCodeEditingTools(intent, context) {
        const requirements = [];
        requirements.push({
            toolId: 'file_editor',
     ...
     * 解析调试工具
     */
    async parseDebuggingTools(intent, context) {
        const requirements = [];
        requirements.push({
     ...
     * 解析测试工具
     */
    async parseTestingTools(intent, context) {
        const requirements = [];
        requirements.push({
            toolId: 'test_runner',
     ...
     * 解析重构工具
     */
    async parseRefactoringTools(intent, context) {
        const requirements = [];
        requirements.push({
            toolId: 'refactoring_tool',
            toolName: 'Refactoring Tool',
     ...
     * 解析优化工具
     */
    async parseOptimizationTools(intent, context) {
        const requirements = [];
        requirements.push({
     ...
     * 提取工具参数
     */
    async extractToolParameters(toolId, userMessage, intent, context) {
        // 简化实现：根据工具ID返回基本参数
        const baseParams = {
     ...
     * 解决工具依赖关系
     */
    resolveToolDependencies(requirements) {
        // 建立依赖关系图
...
exports.CodeMasterQueryEngine = CodeMasterQueryEngine;
// ==================== 状态记忆类 ====================
/**
 * 状态记忆管理器
 */
...
exports.default = CodeMasterQueryEngine;
//# sourceMappingURL=CodeMasterQueryEngine.js.map