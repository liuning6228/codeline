...
exports.CollaborationExample = void 0;
exports.runCollaborationDemos = runCollaborationDemos;
const path = __importStar(require("path"));
const fs = __importStar(require("fs"));
const CollaborationEngine_1 = require("../CollaborationEngine");
...
class CollaborationExample {
    ...
    async demoSetup() {
        ...
        try {
            ...
            const collaborationEngine = (0, CollaborationEngine_1.createCollaborationEngine)(this.workspaceRoot, fileEditor, codeAnalyzer, performanceMonitor, {
    ...
    async demoSessionManagement() {
        ...
        try {
            const fileEditor = new FileEditorTool_1.FileEditorTool();
            const codeAnalyzer = new CodeAnalysisTool_1.CodeAnalysisTool();
            const collaborationEngine = (0, CollaborationEngine_1.createCollaborationEngine)(this.workspaceRoot, fileEditor, codeAnalyzer);
            const testUsers = this.createTestUsers();
            ...
            const session = await collaborationEngine.createSession('Code Review: User Authentication', owner, {
...
`;
            ...
            // 创建会话
            const session = await collaborationEngine.createSession('Code Review Demo', owner, { projectPath: this.testFilesDir });
            await collaborationEngine.joinSession(session.id, reviewer);
            // 步骤1: 添加代码评论
            steps.push({
            ...
            const comment1 = await collaborationEngine.addComment(session.id, 'user-auth.ts', {
                lineStart: 13,
                lineEnd: 13,
                columnStart: 1,
                columnEnd: 1,
            }, 'This comment should mention that storing plain password is insecure', reviewer);
            steps.push({
            ...
            const comment2 = await collaborationEngine.addComment(session.id, 'user-auth.ts', {
                lineStart: 7,
                lineEnd: 7,
                columnStart: 1,
                columnEnd: 1,
            }, 'Should add validation for password strength', reviewer);
            const comment3 = await collaborationEngine.addComment(session.id, 'user-auth.ts', {
                lineStart: 10,
                lineEnd: 10,
                columnStart: 1,
                columnEnd: 1,
            }, 'TODO comment should be more specific', reviewer);
            // 步骤3: 获取文件评论
            steps.push({
            ...
            const resolvedComment = await collaborationEngine.resolveComment(session.id, comment1.id, owner);
            steps.push({
...
`;
            fs.writeFileSync(testFilePath, testContent, 'utf8');
            // 创建会话
            const session = await collaborationEngine.createSession('Edit Suggestions Demo', owner, { projectPath: this.testFilesDir });
            await collaborationEngine.joinSession(session.id, reviewer);
            // 步骤1: 添加带编辑建议的评论
            steps.push({
            ...
            const comment = await collaborationEngine.addComment(session.id, 'utils.ts', {
                lineStart: 3,
                lineEnd: 7,
                columnStart: 1,
                columnEnd: 1,
            }, 'Consider using Array.reduce() for better readability', reviewer);
            // 步骤2: 添加编辑建议
            steps.push({
                name: 'Suggest Edit',
            ...
            const suggestedEdit = await collaborationEngine.suggestEdit(session.id, comment.id, 'replace', reviewer, '  let sum = 0;\n  for (let i = 0; i < items.length; i++) {\n    sum += items[i];\n  }', '  return items.reduce((sum, item) => sum + item, 0);');
            steps.push({
            ...
            const applyResult = await collaborationEngine.applyEdit(session.id, comment.id, owner);
            steps.push({
    ...
    async demoPermissions() {
        ...
        try {
            ...
            // 创建会话，设置严格的权限
            const session = await collaborationEngine.createSession('Permission Demo', owner, {
                projectPath: this.testFilesDir,
                settings: {
            ...
            try {
                await collaborationEngine.addComment(session.id, 'test.ts', { lineStart: 1, lineEnd: 1, columnStart: 1, columnEnd: 1 }, 'Owner comment', owner);
                steps.push({
            ...
            try {
                await collaborationEngine.addComment(session.id, 'test.ts', { lineStart: 1, lineEnd: 1, columnStart: 1, columnEnd: 1 }, 'Reviewer comment', reviewer);
                steps.push({
            ...
            try {
                await collaborationEngine.addComment(session.id, 'test.ts', { lineStart: 1, lineEnd: 1, columnStart: 1, columnEnd: 1 }, 'Viewer comment', viewer);
                steps.push({
            ...
            // 创建另一个会话，设置不同的权限
            const openSession = await collaborationEngine.createSession('Open Session', owner, {
                projectPath: this.testFilesDir,
                settings: {
                    editPermissions: 'all',
            ...
            try {
                await collaborationEngine.addComment(openSession.id, 'test.ts', { lineStart: 1, lineEnd: 1, columnStart: 1, columnEnd: 1 }, 'Viewer comment in open session', viewer);
                steps.push({
    ...
    async demoDataPersistence() {
        ...
        try {
            ...
            const fileEditor = new FileEditorTool_1.FileEditorTool();
            const codeAnalyzer = new CodeAnalysisTool_1.CodeAnalysisTool();
            const engine1 = (0, CollaborationEngine_1.createCollaborationEngine)(this.workspaceRoot, fileEditor, codeAnalyzer, undefined, {
                dataStoragePath: dataDir,
                autoSaveInterval: 1000,
            });
            const testUsers = this.createTestUsers();
            const owner = testUsers[0];
            const session = await engine1.createSession('Persistence Test', owner, { projectPath: this.testFilesDir });
            await engine1.addComment(session.id, 'test.ts', { lineStart: 1, lineEnd: 1, columnStart: 1, columnEnd: 1 }, 'Test comment for persistence', owner);
            ...
            const engine2 = (0, CollaborationEngine_1.createCollaborationEngine)(this.workspaceRoot, fileEditor, codeAnalyzer, undefined, {
                dataStoragePath: dataDir,
            });
            // 步骤5: 验证数据加载
            steps.push({
...
`;
            ...
            const owner = testUsers[0];
            // 步骤3: 创建协作会话
            const session = await collaborationEngine.createSession('Integration Test', owner, { projectPath: this.testFilesDir });
            ...
            if (analysisResult.success && analysisResult.issueStats.total > 0) {
                const comment = await collaborationEngine.addComment(session.id, 'integration-test.ts', { lineStart: 3, lineEnd: 3, columnStart: 1, columnEnd: 1 }, `Code analysis found ${analysisResult.issueStats.total} issues. Consider improving code quality.`, owner);
                steps.push({
...
exports.CollaborationExample = CollaborationExample;
/**
 * 运行协作演示
 */
async function runCollaborationDemos(workspaceRoot) {
...
exports.default = CollaborationExample;
//# sourceMappingURL=CollaborationExample.js.map