...
exports.CodeRefactoringEngine = void 0;
exports.createCodeRefactoringEngine = createCodeRefactoringEngine;
...
class CodeRefactoringEngine {
    ...
    performanceMonitor;
    workspaceRoot;
    constructor(workspaceRoot, fileEditor, codeAnalyzer, performanceMonitor) {
     ...
     * 执行重构
     */
    async executeRefactoring(params) {
        ...
        try {
            ...
            if (this.performanceMonitor) {
                this.performanceMonitor.recordToolExecution(`refactoring_${params.type}`, totalTime, true);
            }
            return result;
        }
        catch (error) {
     ...
     * 重命名标识符
     */
    async renameIdentifier(filePath, oldName, newName, options = {}) {
        // 分析标识符位置
     ...
     * 提取方法
     */
    async extractMethod(filePath, location, methodName, options = {}) {
        const params = {
            type: 'extract_method',
            scope: {
                filePath,
     ...
     * 内联变量
     */
    async inlineVariable(filePath, variableName, options = {}) {
        // 查找变量位置
     ...
     * 预览重构
     */
    async previewRefactoring(params) {
        // 只进行分析和预览，不实际执行
        const impactAnalysis = await this.analyzeImpact(params);
     ...
     * 分析重命名影响
     */
    async analyzeRenameImpact(params, fileAnalysis, projectAnalysis) {
        ...
        // 确定影响范围
        const directlyAffected = [filePath];
        const indirectlyAffected = [];
        const unaffected = [];
        // 如果递归，查找相关文件
        if (recursive) {
            // 简化实现：查找导入该文件的文件
     ...
     * 分析提取方法影响
     */
    async analyzeExtractMethodImpact(params, fileAnalysis, projectAnalysis) {
        const { scope, config } = params;
        const { filePath } = scope;
     ...
     * 分析内联变量影响
     */
    async analyzeInlineVariableImpact(params, fileAnalysis, projectAnalysis) {
        const { scope } = params;
        const { filePath } = scope;
        // 简化实现
        return {
     ...
     * 创建默认影响分析
     */
    createDefaultImpactAnalysis(filePath) {
        return {
            scope: {
                directlyAffected: [filePath],
                indirectlyAffected: [],
     ...
     * 验证安全
     */
    async validateSafety(params, impactAnalysis) {
     ...
     * 执行重构
     */
    async performRefactoring(params) {
        const { type, scope, config } = params;
     ...
     * 执行重命名重构
     */
    async performRenameRefactoring(params, changes) {
        const { scope, config } = params;
        ...
        changes.push({
            ...
            operation: 'replace',
            location,
            oldContent: fileContent.substring(this.getOffset(lines, location.lineStart), this.getOffset(lines, location.lineEnd)),
            newContent: newFileContent.substring(this.getOffset(lines, location.lineStart), this.getOffset(lines, location.lineEnd)),
        });
        // 2. 更新文件
     ...
     * 执行提取方法重构
     */
    async performExtractMethodRefactoring(params, changes) {
        const { scope, config } = params;
        const { filePath, location } = scope;
        const { methodName } = config;
        if (!methodName) {
...
${newMethod}`;
     ...
     * 执行内联变量重构
     */
    async performInlineVariableRefactoring(params, changes) {
        const { scope, config } = params;
        const { filePath, location } = scope;
     ...
     * 模拟重构（用于预览）
     */
    async simulateRefactoring(params) {
        // 返回模拟变更
        return [{
     ...
     * 创建预览
     */
    async createPreview(params, changes, impactAnalysis) {
    ...
    calculateStatistics(changes) {
        ...
        // 简化实现
        const identifiersRenamed = changes.filter(c => c.oldContent && c.newContent && c.oldContent !== c.newContent).length;
        return {
            filesModified,
            linesModified,
            identifiersRenamed,
     ...
     * 生成建议
     */
    generateSuggestions(params, impactAnalysis, safetyValidation) {
        const suggestions = [];
        // 基于影响分析的建议
     ...
     * 查找标识符位置
     */
    async findIdentifierLocation(filePath, identifierName) {
        // 简化实现：返回文件的第一个位置
     ...
     * 查找变量位置
     */
    async findVariableLocation(filePath, variableName) {
        // 使用相同的实现
     ...
     * 查找标识符使用
     */
    async findIdentifierUses(filePath, location) {
        // 简化实现：只返回当前位置
     ...
     * 检查文件是否导入另一个文件
     */
    fileImports(filePath, importedFilePath) {
        // 简化实现
        try {
            const content = fs.readFileSync(filePath, 'utf8');
     ...
     * 检查标识符冲突
     */
    hasIdentifierConflict(filePath, identifierName) {
        // 简化实现
        try {
            const content = fs.readFileSync(filePath, 'utf8');
            // 检查是否已经存在相同的标识符（简化检查）
     ...
     * 分析测试影响
     */
    analyzeTestImpact(filePath, identifierUses) {
        // 简化实现
        return {
            testsToUpdate: [],
            testsMayFail: [],
     ...
     * 分析依赖影响
     */
    analyzeDependencyImpact(filePath, identifierUses) {
        // 简化实现
        return {
            internal: [],
            external: [],
     ...
     * 提取标识符名称
     */
    async extractIdentifierName(filePath, location) {
        try {
            const content = fs.readFileSync(filePath, 'utf8');
            const lines = content.split('\n');
...
exports.CodeRefactoringEngine = CodeRefactoringEngine;
/**
 * 创建代码重构引擎
 */
function createCodeRefactoringEngine(workspaceRoot, fileEditor, codeAnalyzer, performanceMonitor) {
    return new CodeRefactoringEngine(workspaceRoot, fileEditor, codeAnalyzer, performanceMonitor);
}
exports.default = CodeRefactoringEngine;
//# sourceMappingURL=CodeRefactoringEngine.js.map