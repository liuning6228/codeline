...
class EnhancedPromptEngine {
     ...
     */
    generateEnhancedPrompt(task, context, options = {}) {
    ...
    generateContextSection(context) {
        ...
        if (context.projectType) {
            ...
            if (context.language)
                contextSection += `- **Primary Language**: ${context.language}\n`;
            if (context.framework)
                contextSection += `- **Framework**: ${context.framework}\n`;
            contextSection += '\n';
        }
        // Code style
        ...
        if (context.relevantContext?.recentChanges) {
            const recentChanges = context.relevantContext.recentChanges.slice(0, 5);
...
- **Extension**: CodeLine v0.3.0`;
    ...
    getJavaTaskPrompt(task, context) {
...
**Note**: Ensure backward compatibility and follow existing project structure.`;
    ...
    getPythonTaskPrompt(task, context) {
        return `# TASK: ${task}
...
**Note**: Consider performance for large datasets, use appropriate libraries (NumPy, Pandas for data tasks).`;
    ...
    getTypeScriptTaskPrompt(task, context) {
...
**Note**: Consider bundle size and performance implications.`;
    ...
    getJavaScriptTaskPrompt(task, context) {
        return `# TASK: ${task}
...
**Note**: Consider browser compatibility if targeting web applications.`;
    ...
    getReactTaskPrompt(task, context) {
        return `# TASK: ${task}
...
**Note**: Consider Vue 3 migration if applicable, and performance optimization.`;
    ...
    getGeneralTaskPrompt(task, context) {
        return `# TASK: ${task}
...
//# sourceMappingURL=enhancedPromptEngine.js.map