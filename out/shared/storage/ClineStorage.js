...
exports.ClineInMemoryStorage = exports.ClineSyncStorage = exports.ClineStorage = void 0;
...
class ClineStorage {
    ...
    async store(key, value) {
        try {
            ...
            Logger_1.Logger.error(`[${this.name}] failed to store '${key}':`, error);
        }
    }
    /**
...
exports.ClineStorage = ClineStorage;
class ClineSyncStorage {
    /**
     * List of subscribers to storage change events.
     */
    changeSubscribers = [];
    ...
    fireChange(key) {
        for (const subscriber of this.changeSubscribers) {
            try {
                ...
                Logger_1.Logger.error(`[${this.name}] change subscriber error for '${key}':`, error);
            }
        }
    }
    get(key, defaultValue) {
        try {
            ...
            Logger_1.Logger.error(`[${this.name}] failed to get '${key}':`, error);
            return defaultValue;
        }
    }
    /**
    ...
    set(key, value) {
        try {
            ...
            Logger_1.Logger.error(`[${this.name}] failed to set '${key}':`, error);
        }
    }
    delete(key) {
        try {
            ...
            Logger_1.Logger.error(`[${this.name}] failed to delete '${key}':`, error);
        }
    }
    keys() {
        try {
            ...
            Logger_1.Logger.error(`[${this.name}] failed to get keys:`, error);
            return [];
        }
    }
}
exports.ClineSyncStorage = ClineSyncStorage;
/**
 * A simple in-memory implementation of ClineStorage using a Map.
 */
class ClineInMemoryStorage extends ClineStorage {
    /**
...
exports.ClineInMemoryStorage = ClineInMemoryStorage;
