...
exports.SyncWorker = exports.SEVEN_DAYS_MS = void 0;
exports.getBlobStoreSettingsFromEnv = getBlobStoreSettingsFromEnv;
exports.initSyncWorker = initSyncWorker;
exports.disposeSyncWorker = disposeSyncWorker;
 ...
 */
function getBlobStoreSettingsFromEnv() {
    return {
        ...
        maxFailedAgeMs: parseIntEnv(process.env.CLINE_STORAGE_SYNC_MAX_FAILED_AGE_MS, exports.SEVEN_DAYS_MS),
        backfillEnabled: process.env.CLINE_STORAGE_SYNC_BACKFILL_ENABLED === "true",
    };
}
var WorkerEvent;
(function (WorkerEvent) {
    WorkerEvent["WorkerSyncStarted"] = "sync_started";
    WorkerEvent["WorkerSyncCompleted"] = "sync_completed";
    WorkerEvent["WorkerItemSynced"] = "item_synced";
    WorkerEvent["WorkerItemFailed"] = "item_failed";
    WorkerEvent["WorkerStarted"] = "worker_started";
    WorkerEvent["WorkerStopped"] = "worker_stopped";
...
class SyncWorker {
    ...
    constructor(queue, options) {
        ...
        this.options = {
            ...
            maxFailedAgeMs: options.maxFailedAgeMs ?? exports.SEVEN_DAYS_MS,
    ...
    emit(event) {
        for (const listener of this.listeners) {
            try {
                ...
                Logger_1.Logger.error("SyncWorker event listener error:", err);
            }
        }
    }
    /**
    ...
    start() {
        ...
        this.processQueue().catch((err) => {
            Logger_1.Logger.error("SyncWorker initial process error:", err);
        });
    }
    /**
        ...
        try {
            ...
            for (const item of batch) {
                ...
                try {
                    ...
                    await ClineBlobStorage_1.blobStorage.store(`tasks/${this.options.userDistinctId}/${item.taskId}/${item.key}`, data);
                    ...
                    Logger_1.Logger.error(`Failed to sync ${item.taskId}/${item.key}:`, err);
                }
            }
 ...
 */
function initSyncWorker(queue, options) {
    if (workerInstance) {
 ...
 */
async function disposeSyncWorker() {
