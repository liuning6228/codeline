...
class SyncQueue {
    ...
    load() {
        try {
            ...
            Logger_1.Logger.error("[SyncQueue] Failed to load queue data:", error);
            this.data = { items: {} };
        }
    }
    /**
    ...
    flush() {
        ...
        try {
            ...
            Logger_1.Logger.error("[SyncQueue] Failed to write queue data:", error);
        }
    }
    /**
     ...
     */
    cleanupOldSynced(maxAgeMs = worker_1.SEVEN_DAYS_MS) {
     ...
     * @returns Number of items cleaned up
     */
    cleanupFailedItems(maxRetries = 5, maxAgeMs = worker_1.SEVEN_DAYS_MS) {
        const cutoff = Date.now() - maxAgeMs;
        let count = 0;
        Object.entries(this.data.items).forEach(([key, item]) => {
    ...
    enforceMaxSize(maxSize = 1000) {
        ...
        if (keysToRemove.length > 0) {
            this.scheduleWrite();
            Logger_1.Logger.warn(`[SyncQueue] Evicted ${keysToRemove.length} items to enforce max size of ${maxSize}`);
        }
