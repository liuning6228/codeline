...
class PluginSystemIntegrationTest {
     ...
     * 测试插件系统初始化
     */
    async testPluginSystemInitialization() {
        // 初始化插件工具注册表
        ...
        const initialized = await this.pluginToolRegistry.initializeWithPlugins(this.extensionContext, this.globalContext);
        if (!initialized) {
        ...
        const mcpInitialized = await this.pluginMCPHandler.initialize(this.globalContext.workspaceRoot, this.pluginToolRegistry);
        if (!mcpInitialized) {
     ...
     * 测试插件工具注册
     */
    async testPluginToolRegistration() {
     ...
     * 测试插件MCP集成
     */
    async testPluginMCPIntegration() {
     ...
     * 测试插件生命周期管理
     */
    async testPluginLifecycleManagement() {
        if (!this.pluginToolRegistry) {
            throw new Error('Plugin Tool Registry not initialized');
        }
        const pluginManager = this.pluginToolRegistry.getPluginManager();
    ...
    async generateTestReport(startTime) {
        ...
        this.outputChannel.appendLine('\n📊 =================================');
        // 将报告保存到文件
        const reportPath = path.join(this.globalContext.workspaceRoot, '.codeline', 'plugin-system-test-report.json');
        const reportData = {
 ...
 * 运行插件系统集成测试
 */
async function runPluginSystemIntegrationTest(extensionContext, workspaceRoot) {
    const tester = new PluginSystemIntegrationTest(extensionContext, workspaceRoot);
    return await tester.runAllTests();
}
//# sourceMappingURL=PluginSystemIntegrationTest.js.map