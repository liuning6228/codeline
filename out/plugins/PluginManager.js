...
class PluginManager {
    ...
    extensionContext;
    lifecycleListeners = [];
    constructor(extensionContext, globalContext, config) {
    ...
    getState() {
        ...
        return {
            ...
            activePlugins: Array.from(this.pluginStates.values()).filter(state => state === PluginInterface_1.PluginLifecycleState.ACTIVATED).length,
            pluginStates,
        };
    }
    /**
     ...
     * 加载插件配置
     */
    async loadPluginConfig(pluginPath, pluginId) {
     ...
     * 检查插件依赖
     */
    async checkPluginDependencies(pluginId, dependencies) {
