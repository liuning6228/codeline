...
const enhancedPromptEngine_1 = require("../prompt/enhancedPromptEngine");
...
class TaskEngine {
    ...
    steps = [];
    isExecuting = false;
    pendingDiffs = new Map(); // diffId -> FileDiff
    approvalWorkflow;
    // Plan/Act mode state
    currentMode = 'act';
    onModeChange;
     ...
     * 分析项目上下文（使用增强分析如果可用）
     */
    async analyzeProjectWithContext(currentFile) {
        // 尝试使用增强分析
        if ('analyzeEnhancedWorkspace' in this.projectAnalyzer &&
            typeof this.projectAnalyzer.analyzeEnhancedWorkspace === 'function') {
            try {
                const enhancedContext = await this.projectAnalyzer.analyzeEnhancedWorkspace(currentFile);
     ...
     * 开始处理一个新任务
     */
    async startTask(taskDescription, options = {}) {
        if (this.isExecuting) {
            return {
                success: false,
                steps: [],
        ...
        try {
            ...
            const prompt = this.enhancedPromptEngine.generateEnhancedPrompt(taskDescription, projectContext, {
     ...
     * 批准或拒绝待处理的文件差异
     */
    async approveDiff(diffId, action) {
        if (!this.fileManager) {
            return {
                success: false,
    ...
    async handleProblemsCommand() {
        ...
        try {
            ...
            const problems = [];
            // 检查package.json是否存在
            const packageJson = projectContext.files.find((f) => f.endsWith('/package.json') || f === 'package.json');
            if (!packageJson) {
                problems.push('No package.json found - this may not be a Node.js project');
            }
            // 检查常用文件
            const hasSrc = projectContext.files.some((f) => f.includes('src/'));
            if (!hasSrc) {
                problems.push('No src/ directory found - consider organizing code in src/');
            }
            // 检查TypeScript配置文件
            const hasTsConfig = projectContext.files.some((f) => f.endsWith('/tsconfig.json') || f === 'tsconfig.json');
            const hasTypeScriptFiles = projectContext.files.some((f) => f.endsWith('.ts'));
            ...
            if (projectContext.codeQuality) {
                ...
                if (codeQuality.issues && codeQuality.issues.length > 0) {
                    const highSeverityIssues = codeQuality.issues.filter((issue) => issue.severity === 'high');
                    const mediumSeverityIssues = codeQuality.issues.filter((issue) => issue.severity === 'medium');
                ...
                if (codeQuality.suggestions && codeQuality.suggestions.length > 0) {
                    const highConfidenceSuggestions = codeQuality.suggestions.filter((suggestion) => suggestion.confidence > 0.7);
                ...
                if (projectContext.architecturePatterns && projectContext.architecturePatterns.length > 0) {
                    const patternNames = projectContext.architecturePatterns.map((pattern) => pattern.name).join(', ');
    ...
    async handleSnapshotCommand() {
        ...
        try {
            ...
            this.updateStepStatus(0, 'completed');
            const fileTypes = {};
            projectContext.files.forEach((filePath) => {
...
<div class="diff-container">
  ...
  ${diffHtml}
    ...
    async executeTerminalStep(step) {
    ...
    async executeBrowserStep(step) {
        ...
        try {
            // 执行浏览器自动化序列
            const actionsToExecute = actions || [{ type: 'navigate', url }];
     ...
     * 提议一个操作到批准工作流
     */
    proposeOperationApproval(type, description, data, options = {}) {
        const itemId = this.approvalWorkflow.proposeOperation(type, description, data, {
            autoExecute: options.autoExecute,
            autoApproveDelay: options.autoExecute ? 0 : undefined
        });
        // 更新相关步骤状态（如果有关联）
        const stepIndex = this.steps.findIndex(step => step.description.includes(description.substring(0, 50)));
        if (stepIndex >= 0) {
     ...
     * 批准一个操作
     */
    async approveOperation(itemId, reason) {
     ...
     * 生成待批准项的HTML摘要
     */
    generateApprovalsHtml() {
        const pending = this.getPendingApprovals();
        if (pending.length === 0) {
            return '<div class="no-pending-approvals">✅ No pending approvals</div>';
        }
        const itemsHtml = pending.map(item => this.approvalWorkflow.generateHtmlSummary(item)).join('');
        return `
