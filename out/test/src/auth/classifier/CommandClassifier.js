...
exports.createCommandClassifier = createCommandClassifier;
/**
 * 命令分类器
 */
class CommandClassifier {
    model = null; // 在实际实现中会加载ML模型
    rules = [];
    constructor() {
     ...
     * 批量分类
     */
    async classifyBatch(commands, context) {
        const results = [];
     ...
     * 添加自定义规则
     */
    addRule(pattern, type, riskLevel, readOnly = false, action = 'ask', sandboxLevel = 'medium') {
    ...
    async classifyWithModel(command, context) {
    ...
    defaultClassification(command, context) {
        const keywords = this.extractKeywords(command);
        // 默认分类：中等风险，需要确认
        return {
            type: 'other',
    ...
    createPatternFromCommand(command) {
        // 将命令转换为正则表达式模式
 ...
 * 创建命令分类器实例
 */
function createCommandClassifier() {
    return new CommandClassifier();
}
//# sourceMappingURL=CommandClassifier.js.map