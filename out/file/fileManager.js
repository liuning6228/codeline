...
class FileManager {
     ...
     * 编辑文件（带差异计算）- 立即写入模式
     */
    async editFileWithDiff(filePath, newContent) {
        try {
            const fullPath = this.resolvePath(filePath);
            // 读取现有内容
     ...
     */
    async prepareFileEdit(filePath, newContent) {
        try {
            const fullPath = this.resolvePath(filePath);
            // 读取现有内容
            const oldContent = await this.readFileIfExists(fullPath);
     ...
     * 读取文件内容（带行数限制）
     */
    async readFileWithLimits(filePath, offset = 1, limit = 1000) {
        try {
            const fullPath = this.resolvePath(filePath);
    ...
    async refreshFileInEditor(filePath) {
        try {
            const uri = vscode.Uri.file(filePath);
    ...
    generateDiffSummary(oldContent, newContent) {
        ...
        if (modified > 0)
            parts.push(`${modified} lines modified`);
        if (parts.length === 0) {
            return 'No changes';
        }
        return parts.join(', ');
     ...
     * 检查文件名是否匹配模式（支持简单通配符）
     */
    matchesPattern(filename, pattern, caseSensitive = false) {
        let fn = filename;
        let pat = pattern;
        if (!caseSensitive) {
            fn = fn.toLowerCase();
     ...
     * 在文件内容中搜索文本
     */
    searchInContent(content, searchText, caseSensitive = false) {
        const results = [];
        const lines = content.split('\n');
