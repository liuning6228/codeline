...
import { 
  ...
  MCPResponse,
  MCPToolRegistry
} from '../../src/mcp/MCPHandler';
import { MCPTool } from '../../src/mcp/mcpManager';
...
describe('MCP端到端集成测试', () => {
  ...
  beforeEach(async () => {
    ...
    mcpHandler = new MCPHandler(mockContext);
  ...
  describe('工具发现和注册', () => {
    ...
    it('应该支持动态工具发现', async () => {
      ...
      // 创建一个新工具
      const newTool = new MockTool(
        'new_dynamic_tool',
        '动态工具',
        '动态添加的工具',
        [],
        async () => ({ success: true, message: '动态工具执行成功' })
      );
  ...
  describe('工具执行流程', () => {
    ...
    it('应该正确处理工具参数验证', async () => {
      ...
      try {
        ...
        assert.ok(error.message.includes('路径参数是必需的') || error.message.includes('参数验证失败'), 
                  `应该返回参数验证错误，实际：${error.message}`);
      }
      ...
      assert.ok(result.error.includes('ENOENT') || result.error.includes('文件'), 
                `应该返回文件不存在错误，实际：${result.error}`);
    });
...
class Calculator {
  ...
  describe('错误处理和恢复', () => {
    ...
    it('应该处理网络超时', async () => {
      // 创建一个模拟网络超时的工具
      const timeoutTool = new MockTool(
        'timeout_tool',
        '超时工具',
        '模拟网络超时的工具',
        [],
      ...
      try {
        ...
        assert.ok(error.message.includes('超时') || error.message.includes('timeout'), 
                  `应该返回超时错误，实际：${error.message}`);
      }
    });
  });
  ...
  describe('性能和监控', () => {
    ...
    it('应该缓存工具结果', async () => {
      ...
      const cachedTool = new MockTool(
        'cached_tool',
        '缓存工具',
        '支持结果缓存的工具',
        [{ name: 'input', type: 'string', description: '输入值' }],
        async (params: any) => {
          executionCount++;
          return { 
            success: true, 
  ...
  describe('安全性和权限', () => {
    it('应该验证工具执行权限', async () => {
      ...
      try {
        ...
        assert.ok(error.message.includes('找不到') || error.message.includes('不存在'), 
                  `应该返回工具不存在错误，实际：${error.message}`);
      }
    });

    it('应该限制危险工具访问', async () => {
      // 创建一个危险工具
      const dangerousTool = new MockTool(
        'dangerous_tool',
        '危险工具',
        '可能执行危险操作的工具',
        [{ name: 'command', type: 'string', description: '系统命令' }],
        async (params: any) => {
          // 模拟执行系统命令
