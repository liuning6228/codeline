...
import { PermissionDialog } from '../../../src/auth/ui/PermissionDialog';
import { PermissionDialogOptions, PermissionDialogResult } from '../../../src/auth/ui/PermissionDialog';
...
describe('PermissionDialog 综合测试套件', () => {
  let permissionDialog: PermissionDialog;

  beforeEach(() => {
    // 获取PermissionDialog实例（通常是单例）
    permissionDialog = PermissionDialog.getInstance();
  ...
  describe('1. 基础功能测试', () => {
    it('应该获取PermissionDialog实例', () => {
      assert.ok(permissionDialog, 'PermissionDialog实例应该被获取');
      assert.strictEqual(typeof (permissionDialog as any).showPermissionDialog, 'function', '应该有showPermissionDialog方法');
    });

    it('应该验证实例是单例', () => {
      const instance1 = PermissionDialog.getInstance();
      const instance2 = PermissionDialog.getInstance();
  ...
  describe('2. 对话框选项验证', () => {
    it('应该接受有效的对话框选项', () => {
      const validOptions: PermissionDialogOptions = {
        toolName: '测试工具',
        toolId: 'test-tool',
    ...
    it('应该处理缺少可选字段的选项', () => {
      const minimalOptions: PermissionDialogOptions = {
        toolName: '最小工具',
        toolId: 'minimal-tool',
  ...
  describe('3. 用户选择流程模拟', () => {
    it('应该模拟用户允许操作', async () => {
      const testOptions: PermissionDialogOptions = {
        toolName: '测试工具',
        toolId: 'test-tool',
      ...
      const mockResult: PermissionDialogResult = {
        choice: 'allow',
  ...
  describe('4. 规则学习和建议生成', () => {
    it('应该为低风险操作生成学习建议', () => {
      const lowRiskOptions: PermissionDialogOptions = {
        toolName: '安全工具',
        toolId: 'safe-tool',
    ...
    it('应该为高风险操作生成学习建议', () => {
      const highRiskOptions: PermissionDialogOptions = {
        toolName: '危险工具',
        toolId: 'danger-tool',
    ...
    it('应该为中风险操作生成学习建议', () => {
      const mediumRiskOptions: PermissionDialogOptions = {
        toolName: '需要确认的工具',
        toolId: 'confirm-tool',
  ...
  describe('5. 上下文感知决策', () => {
    it('应该考虑用户角色上下文', async () => {
      ...
      // 模拟管理员的选择
      const adminResult: PermissionDialogResult = {
        choice: 'allow',
        rememberChoice: true,
        ruleType: 'category',
        feedback: '管理员允许此操作'
      ...
      // 模拟普通用户的选择
      const guestResult: PermissionDialogResult = {
        choice: 'deny',
        rememberChoice: false,
    ...
    it('应该考虑环境上下文', async () => {
      ...
      for (const ctx of contexts) {
        ...
        // 模拟不同环境的选择
        let mockResult: PermissionDialogResult;
  ...
  describe('6. 边界情况和错误处理', () => {
    it('应该处理并发对话框请求', async () => {
      ...
      for (let i = 0; i < concurrentRequests; i++) {
        ...
        promises.push(Promise.resolve({
          requestId: i,
          result: { choice: i % 2 === 0 ? 'allow' : 'deny' } as PermissionDialogResult
        }));
      }
    ...
    it('应该处理对话框超时', async () => {
      const timeoutOptions: PermissionDialogOptions = {
        toolName: '超时测试工具',
        toolId: 'timeout-tool',
      ...
      // 模拟超时情况
      const timeoutResult: PermissionDialogResult = {
    ...
    it('应该处理无效的风险等级', async () => {
      ...
      invalidRiskOptions.forEach((options, index) => {
        ...
        // 模拟处理无效风险等级
        const normalizedRisk = Math.max(0, Math.min(10, 
          typeof options.riskLevel === 'number' && !isNaN(options.riskLevel) 
            ? options.riskLevel 
            : 5
        ));
  ...
  describe('7. 集成场景测试', () => {
    it('应该与RuleManager集成工作', async () => {
      ...
      workflow.forEach((step, index) => {
        console.log(`  ${index + 1}. ${step.step}:`, 
          step.step === '工具执行请求' ? `${step.toolId}: ${step.command}` :
    ...
    it('应该与EnhancedBashTool集成工作', async () => {
