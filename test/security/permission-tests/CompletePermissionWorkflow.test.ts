...
// 模拟组件（在实际测试中会导入真实组件）
interface MockRuleManager {
  checkPermission(toolId: string, input: string, context: any): any;
  learnFromDecision(decision: any): void;
}

interface MockCommandClassifier {
  classify(command: string, context?: any): Promise<any>;
}

interface MockPermissionDialog {
  showPermissionDialog(options: any): Promise<any>;
}

interface MockEnhancedBashTool {
  checkPermissions(input: any, context: any): Promise<any>;
  execute(input: any, context: any): Promise<any>;
}

// 创建模拟组件
function createMockRuleManager(): MockRuleManager {
  const rules = new Map();
  
  return {
    checkPermission(toolId: string, input: string, context: any) {
      // 简单规则匹配逻辑
    ...
    learnFromDecision(decision: any) {
...
function createMockCommandClassifier(): MockCommandClassifier {
  return {
    async classify(command: string, context?: any) {
      // 简单分类逻辑
      const result: any = {
        type: 'unknown',
        riskLevel: 5,
...
function createMockPermissionDialog(): MockPermissionDialog {
  return {
    async showPermissionDialog(options: any) {
...
function createMockEnhancedBashTool(): MockEnhancedBashTool {
  ...
  return {
    async checkPermissions(input: any, context: any) {
      ...
      const riskLevel = Math.max(
        parsedCommand.semantic.riskLevel,
        classification.riskLevel,
        ruleCheck.matched && ruleCheck.action === 'deny' ? 9 : 5
      );
...
describe('完整权限工作流测试套件', () => {
  let mockRuleManager: MockRuleManager;
  let mockCommandClassifier: MockCommandClassifier;
  let mockPermissionDialog: MockPermissionDialog;
  let mockEnhancedBashTool: MockEnhancedBashTool;
  ...
  describe('1. 基础工作流测试', () => {
    it('应该执行安全命令的完整工作流', async () => {
      ...
      const permissionCheck = await mockEnhancedBashTool.checkPermissions(
        { command: testCommand, description: '列出目录' },
        context
      );
      ...
      if (permissionCheck.allowed && !permissionCheck.requiresUserConfirmation) {
        const executionResult = await mockEnhancedBashTool.execute(
          { command: testCommand },
          context
        );
    ...
    it('应该执行需要确认的命令工作流', async () => {
      ...
      console.log(`步骤1: 检查命令权限 - "${testCommand}"`);
      const permissionCheck = await mockEnhancedBashTool.checkPermissions(
        { command: testCommand, description: '更新包列表' },
        context
      );
    ...
    it('应该拒绝危险命令工作流', async () => {
      ...
      console.log(`步骤1: 检查命令权限 - "${testCommand}"`);
      const permissionCheck = await mockEnhancedBashTool.checkPermissions(
        { command: testCommand, description: '危险删除操作' },
        context
      );
  ...
  describe('2. 复杂场景测试', () => {
    ...
    it('应该处理上下文相关的工作流', async () => {
      ...
      for (const ctx of contexts) {
        ...
        // 权限检查
        const permissionCheck = await mockEnhancedBashTool.checkPermissions(
          { command: testCommand, description: '重启数据库服务' },
          ctx
        );
  ...
  describe('3. 错误处理和边界情况', () => {
    it('应该处理无效命令工作流', async () => {
      console.log('\n=== 测试无效命令工作流 ===');
    ...
    it('应该处理超时命令工作流', async () => {
      ...
      // 权限检查
      const permissionCheck = await mockEnhancedBashTool.checkPermissions(
        { command: longRunningCommand, description: '长时间运行命令', timeout: context.timeout },
        context
      );
    ...
    it('应该处理并发权限检查工作流', async () => {
      ...
      const promises = concurrentCommands.map(async (command, index) => {
        return mockEnhancedBashTool.checkPermissions(
          { command, description: `并发命令 ${index + 1}` },
          context
        );
      });
    ...
    it('应该测试完整工作流的性能', async () => {
      ...
      for (const scenario of testScenarios) {
        ...
        for (const command of scenario.commands) {
          ...
          if (permissionCheck.allowed) allowedCount++;
          if (permissionCheck.requiresUserConfirmation) confirmationCount++;
